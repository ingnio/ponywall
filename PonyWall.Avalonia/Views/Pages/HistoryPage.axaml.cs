...
{
    public partial class HistoryPage : UserControl, INavigablePage
    {
        ...
        /// <summary>
        /// Sets the text filter and triggers a reload. Can be called before
        /// or after the first <see cref="OnNavigatedTo"/> to pre-populate the
        /// filter bar (replaces the old <c>_initialFilter</c> constructor
        /// parameter).
        /// </summary>
        public void SetFilter(string filterText)
        {
            txtFilter.Text = filterText;
            _searchText = (filterText ?? string.Empty).Trim();
            ...
            try
            {
                ...
                await Task.Run(() =>
                {
                    ...
                    written = HistoryExporter.Export(
                        ...
                        stream,
                        toolName: "PonyWall",
                        toolVersion: typeof(HistoryPage).Assembly.GetName().Version?.ToString());
                }).ConfigureAwait(true);
        ...
        private void ShowDetailsPanel()
        {
            if (borderDetails != null)
            {
                ...
                // Add the details column if not present
                if (gridContent.ColumnDefinitions.Count < 2)
                {
                    gridContent.ColumnDefinitions.Add(new ColumnDefinition(8, GridUnitType.Pixel));
                    gridContent.ColumnDefinitions.Add(new ColumnDefinition(380, GridUnitType.Pixel));
                    Grid.SetColumn(borderDetails, 2);
                }
            }
        }

        private void BtnCloseDetails_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
        {
            HideDetailsPanel();
        }
        ...
        {
            if (borderDetails != null)
                borderDetails.IsVisible = false;
            // Remove the details columns so the DataGrid takes full width
            while (gridContent.ColumnDefinitions.Count > 1)
